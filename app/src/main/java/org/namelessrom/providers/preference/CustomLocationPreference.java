...
public class CustomLocationPreference extends EditTextPreference {
    ...
    private class WeatherLocationTask
            extends AsyncTask<Void, Void, List<WeatherProvider.LocationResult>> {
        ...
        @Override
        protected List<WeatherProvider.LocationResult> doInBackground(Void... input) {
            return Preferences.weatherProvider(getContext()).getLocations(mLocation);
        }

        @Override
        protected void onPostExecute(List<WeatherProvider.LocationResult> results) {
        ...
        private void handleResultDisambiguation(
                final List<WeatherProvider.LocationResult> results) {
        ...
        private CharSequence[] buildItemList(List<WeatherProvider.LocationResult> results) {
            ...
            for (WeatherProvider.LocationResult result : results) {
            ...
            for (int i = 0; i < count; i++) {
                WeatherProvider.LocationResult result = results.get(i);
        ...
        private void applyLocation(final WeatherProvider.LocationResult result) {
